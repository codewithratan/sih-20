...
export default function Maintenance() {
  ...
  const status = useMemo(
    () => predictFailure(latest.eff, latest.volt),
    [latest],
  );
  ...
  return (
    <div className="container py-8 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-extrabold leading-tight">
              Smart Solar Pump Monitoring & Predictive Maintenance
            </h1>
            <p className="text-sm text-muted-foreground">
              Real-time metrics, rule-based alerts, mock SMS, and blockchain
              usage logs for DISCOM compliance.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 items-start">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Simulated sensor data updating every second.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-3 gap-3">
              <Metric
                label="Flow (L/min)"
                value={latest.flow.toFixed(1)}
                icon={<Droplets className="h-4 w-4" />}
              />
              <Metric
                label="Efficiency (%)"
                value={latest.eff.toFixed(0)}
                icon={<Activity className="h-4 w-4" />}
              />
              <Metric
                label="Voltage (V)"
                value={latest.volt.toFixed(2)}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Rule-based alerts; Prophet can be added post-MVP.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div
              className={`rounded-md border p-3 text-sm ${status === "Normal" ? "border-green-200 bg-green-50" : "border-amber-200 bg-amber-50"}`}
            >
              <div className="flex items-center gap-2 font-medium">
                <Bell className="h-4 w-4 text-amber-600" /> Status: {status}
              </div>
              <div className="mt-1 text-xs text-muted-foreground">
                Thresholds: efficiency {"<"} 70% or voltage {"<"} 11.5V trigger
                alerts.
              </div>
            </div>
            <Button onClick={sendSms}>Send SMS Alert</Button>
            {smsResponse && (
              <div className="rounded-md border p-3 text-xs font-mono bg-muted/30">
                {smsResponse}
              </div>
            )}
          </CardContent>
      ...
      <div className="grid gap-6 md:grid-cols-2 items-start">
        <ResultsCard
          title="Blockchain Usage Logs"
          description="Simulated Mumbai testnet logs for demonstration."
        >
          <div className="flex items-end gap-3">
            <div className="grid gap-2">
              ...
              <Input
                value={addKWh}
                onChange={(e) =>
                  setAddKWh(e.target.value.replace(/[^\d.]/g, ""))
                }
                inputMode="decimal"
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Maintenance support eligibility (mock).
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 text-sm">
            <div className="rounded-md bg-accent p-3 text-muted-foreground">
              Eligible for ₹5,000 maintenance subsidy; apply at
              pmkusum.mnre.gov.in.
            </div>
            <a
...
function nextTick(data: SamplePoint[]): SamplePoint[] {
  ...
  const next: SamplePoint = {
    t: d.toTimeString().slice(3, 8),
    flow: clamp(
      (data[data.length - 1]?.flow ?? 10) + (Math.random() - 0.5) * 0.4,
      8,
      12,
    ),
    eff: clamp(
      (data[data.length - 1]?.eff ?? 80) + (Math.random() - 0.5) * 1.2,
      60,
      92,
    ),
    volt: clamp(
      (data[data.length - 1]?.volt ?? 12) + (Math.random() - 0.5) * 0.08,
      10.8,
      13.2,
    ),
  };
...
function predictFailure(efficiency: number, voltage: number): string {
  if (efficiency < 70 || voltage < 11.5)
    return "Warning: Clean panels or check battery";
  return "Normal";
}
